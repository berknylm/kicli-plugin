...
| Place a new symbol | `kicli sch <file> place <lib_id> <ref\|?> [<value>] [--at X,Y] [--angle 0\|90\|180\|270] [--mirror x\|y] [--footprint FP]` — adds a symbol from the bundled KiCad catalog. `--at` auto-grids when omitted. Pass `?` as ref to auto-annotate the next free `<PREFIX><N>` (Device:R → R1, R2, …). Inherits the lib's default Footprint/Datasheet/Description. |
| Attach a net (label-first) | `kicli sch <file> net <net-name> <ref>:<pin> [<ref>:<pin> ...] [--as local\|global\|hier\|power]` — drops a label (or power port for known rails: GND, +3V3, +5V, VCC, VDD, VSS, VEE, GNDA, AGND, PGND, +BATT, EARTH, and `+<voltage>V` patterns) at the exact pin position. `--as hier` writes `(hierarchical_label)` for nets crossing into a parent sheet. **No wires are drawn** — matching label text = same net. |
| No-connect marker | `kicli sch <file> nc <ref>:<pin> [<ref>:<pin> ...]` — silences ERC on deliberately-floating pins |
| Hierarchical sub-sheet | `kicli sch <parent> sheet <name> <child-file> [--at X,Y] [--size W,H] [--pins NAME[:type],…]` — adds a sub-sheet block to the parent + creates the child `.kicad_sch` if missing. Pin types: `passive\|input\|output\|bidirectional\|tri_state\|power_in\|power_out`. After this, draw inside the child using `--as hier` for nets that cross the boundary. |
| Layout-readiness check | `kicli sch <file> check [--no-erc]` — single-shot validator: dup refs (incl. #PWR), missing footprints, dup UUIDs, ERC error count. Exit 0 only when layout-ready. Run before opening the project in pcbnew. |
